    ...
    for qubits_in_use in range(2):
        ...
        class HybridFunction(Function):
            ...
            def backward(ctx, grad_output):
                ...
                shift_right = input_list + \
                    np.ones(input_list.shape) * ctx.shift
        ...
        while n_samples_show > 0:
            ...
            axes[n_samples_show -
                 1].imshow(images[0].numpy().squeeze(), cmap='gray')
            axes[n_samples_show - 1].set_xticks([])
            axes[n_samples_show - 1].set_yticks([])
            axes[n_samples_show -
                 1].set_title("Labeled: {}".format(targets.item()))
        ...
        test_loader = torch.utils.data.DataLoader(
            X_test, batch_size=1, shuffle=True)
        ...
        with torch.no_grad():
            ...
            print('Performance on test data:\n\tLoss: {:.4f}\n\tAccuracy: {:.1f}%'.format(
                sum(total_loss) / len(total_loss), accuracy))
        n_samples_show = 6
        count = 0
        fig, axes = plt.subplots(
            nrows=1, ncols=n_samples_show, figsize=(10, 3))
...
qubitsVsAccuracy = {qubitsUsed[i]: accuracyPlural[i]
                    for i in range(len(qubitsUsed))}
...
def export_dict(filename, dict):
    with open(filename, 'w') as f:
        ...
        w.writerow(dict)
    local_file = FileLink(
        filename, result_html_prefix="Click here to download: ")
    display(local_file)
